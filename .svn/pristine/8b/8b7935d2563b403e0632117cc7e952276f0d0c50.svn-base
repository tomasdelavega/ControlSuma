	...
	private HashMap<String, Object>  preparaConsulta(InventarioFilterDTO filtro) {
		...
		try {
			...
			if(!filtro.getCodJira().equalsIgnoreCase("")){
				...
				sentWhere += " UPPER(i.codJira) LIKE :codJira ";
				parametros.put ("codJira","%" + filtro.getCodJira().toUpperCase() + "%");
			}
			...
			if(!filtro.getCodInventario().equalsIgnoreCase("")){
				...
				sentWhere += " UPPER(i.codInventario) LIKE :codInventario ";
				parametros.put("codInventario", "%" + filtro.getCodInventario().toUpperCase() + "%");
			}
